...
class ptBEVnet(nn.Module):
    ...
    def forward(self, pt_fea, xyz_ind, cur_dev):
        ...
        cat_pt_fea = torch.cat(pt_fea, dim=0)  # 所有batch cat起来    (N1+N2+...+Nbs, 17)
        cat_pt_ind = torch.cat(cat_pt_ind, dim=0) # （B,W,H,Z）
        ...
        unq, unq_inv, unq_cnt = torch.unique(cat_pt_ind[:, :3], return_inverse=True, return_counts=True, dim=0) # 把在同一个体素的点的坐标相同的变为同一个，同时保留每一个坐标该去到哪一个体素的索引
        ...
        # 将点云中各个点按照voxle索引值聚合到（480，360，32）分辨率
        ...
        res_data_dim = [len(pt_fea), self.grid_size[0], self.grid_size[1], cat_pt_res_fea.shape[1]]  # 4, 480, 360, 8
        ...
        # run through network
        moving_output,movable_output = self.BEV_model(
            out_data, res_data)
        
        # print("moving_output",moving_output.shape)
        # print("movable_output",movable_output.shape)
