...
def process_one_seq(config,seq):
    # specify parameters
    ...
    if num_frames >= len(poses) or num_frames <= 0:
        print(f'generate training data for all frames with number of: {len(poses)} in {seq}')
    else:
    ...
    for frame_idx in range(len(scan_paths) - num_last_n - num_prev_n + 1, len(scan_paths)):
        ...
        # np.save(file_name, diff_scan)
    print(f"seq {seq} residual finish!")
    print("Done.")
...
if __name__ == '__main__':
    ...
    # load config file
    config_filename = './utils/generate_residual/utils/data_preparing_polar_sequential.yaml'
    ...
    residual_image_folder = config['residual_image_folder']
    polar_image_params = config['polar_image']
    width = polar_image_params['width']
    height = polar_image_params['height']
    max_range = polar_image_params['max_range']
    min_range = polar_image_params['min_range']
    residual_image_folder = residual_image_folder+'-'+str(width)+'-'+str(height)+'-'+str(max_range)+'-'+str(min_range)+'/'
    ...
    # for SemanticKITTI
    for seq in range(0, 11):  # sequences id
        ...
        process = Process(target=process_one_seq, args=(config, seq))  # 指定进程要执行的函数和参数
    ...
    for seq in range(11, 22):  # sequences id
        # Update the value in config to facilitate the iterative loop
        config['scan_folder'] = scan_folder + f"sequences/{'%02d' % seq}/velodyne"
        config['pose_file'] = scan_folder + f"sequences/{'%02d' % seq}/poses.txt"
        config['calib_file'] = scan_folder + f"sequences/{'%02d' % seq}/calib.txt"
        config['residual_image_folder'] = residual_image_folder + f"{'%02d' % seq}/residual_images"
        # ic(config)
        # process_one_seq(config)
        process = Process(target=process_one_seq, args=(config, seq))  # 指定进程要执行的函数和参数
