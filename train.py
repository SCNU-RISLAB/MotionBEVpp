...
def main():
    ...
    save_code(model_save_path=model_save_path,train_code_path="train.py")
    ...
    while epoch < epochs:
        ...
        with tqdm(total=len(train_dataset_loader),desc="train") as pbar_train:
            for i_iter, (train_moving_label, train_movable_label,train_grid, train_pt_labs_moving, train_pt_labs_movable,train_pt_fea) in enumerate(train_dataset_loader):
                ...
                if i_iter % eval_iter == 0 and i_iter != 0 and epoch >= train_cfg['eval_init_epoch'] :
                    ...
                    with torch.no_grad():
                        with tqdm(total=len(val_dataset_loader),desc='eval') as pbar_eval:
                            for i_iter_val, (val_moving_label,val_movable_label, val_grid, val_pt_labs_moving, val_pt_labs_movable,val_pt_fea) in enumerate(val_dataset_loader):
